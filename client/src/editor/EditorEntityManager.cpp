...
namespace RType {
    namespace Client {
        ...
        EditorEntityManager::EditorEntityManager(Renderer::IRenderer* renderer,
            ...
            , m_registry(registry)
            , m_assets(assets)
            , m_colliderManager(std::make_unique<EditorColliderManager>(renderer))
        {
        }
        ...
        EditorEntityData* EditorEntityManager::PlaceEntity(const EditorPaletteSelection& selection, const Math::Vector2& worldPos) {
            ...
            initializeDefaultCollider(data);
        ...
        bool EditorEntityManager::SelectAt(const Math::Vector2& worldPos) {
            ...
            for (int i = static_cast<int>(m_entities.size()) - 1; i >= 0; --i) {
                ...
                if (EditorGeometry::PointInEntityBounds(worldPos, entity.x, entity.y, entity.scaleWidth, entity.scaleHeight)) {
        ...
        void EditorEntityManager::RebuildDefaultCollider(EditorEntityData& data) {
            initializeDefaultCollider(data);
        }
        ...
        void EditorEntityManager::drawOutline(const EditorEntityData& entity) const {
            Math::Rectangle rect = EditorGeometry::BuildRect(
                entity.x - entity.scaleWidth / 2.0f,
                entity.y - entity.scaleHeight / 2.0f,
                entity.scaleWidth,
                entity.scaleHeight);

            EditorDrawing::DrawRectangleOutline(m_renderer, rect,
                Selection::OUTLINE_COLOR, Selection::OUTLINE_THICKNESS);
        }
        ...
        void EditorEntityManager::DrawColliders(int selectedColliderIndex) const {
            if (m_colliderManager) {
                m_colliderManager->DrawColliders(GetSelectedEntity(), selectedColliderIndex);
            }
        }

        void EditorEntityManager::DrawColliderHandles(int colliderIndex) const {
            if (m_colliderManager) {
                m_colliderManager->DrawHandles(GetSelectedEntity(), colliderIndex);
            }
        }

        ColliderHandle EditorEntityManager::GetColliderHandleAt(const Math::Vector2& worldPos) const {
            if (m_colliderManager) {
                return m_colliderManager->GetHandleAt(GetSelectedEntity(), worldPos);
            }
            return {};
        }
        ...
        void EditorEntityManager::AddCollider(const Math::Vector2& worldPos) {
            ...
            if (!entity || !m_colliderManager) {
                return;
            }

            m_selectedColliderIndex = m_colliderManager->AddCollider(*entity, worldPos);
            SyncEntity(*entity);
        }
        ...
        bool EditorEntityManager::RemoveCollider(int colliderIndex) {
            ...
            if (m_colliderManager->RemoveCollider(*entity, colliderIndex)) {
                SyncEntity(*entity);

                // Update selected collider index
        ...
        void EditorEntityManager::ResizeCollider(int colliderIndex, ColliderHandle::Type handleType, const Math::Vector2& worldPos) {
            ...
            m_colliderManager->ResizeCollider(*entity, colliderIndex, handleType, worldPos);
            SyncEntity(*entity);
        }

        void EditorEntityManager::initializeDefaultCollider(EditorEntityData& data) const {
        ...
        void EditorEntityManager::createColliderEntities(EditorEntityData& data) {
            data.colliderEntities.clear();
